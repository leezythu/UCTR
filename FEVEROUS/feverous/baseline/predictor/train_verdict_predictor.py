...
from transformers import BertForSequenceClassification, Trainer, TrainingArguments, AdamW, BertTokenizer, BertPreTrainedModel, BertModel,  RobertaForSequenceClassification, RobertaTokenizer,RobertaModel
...
def model_trainer(model_path,pretrained_path,num_label, train_dataset, test_dataset=None):
    print("num_label",num_label)
    print("model_path",model_path)
    print("pretrained_path:",pretrained_path)
    # pretrained_path = 'ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli'
    model = RobertaForSequenceClassification.from_pretrained(pretrained_path,ignore_mismatched_sizes=True,num_labels =num_label, return_dict=True,)#ynie/roberta-large-snli_mnli_fever_anli_R1_R2_R3-nli


    training_args = TrainingArguments(
    output_dir=model_path,          # output directory
    num_train_epochs=2000,              # total # of training epochs
...
def claim_evidence_predictor(annotations_train, annotations_dev, args):
    # if not os.path.exists("text_test_verdict"):
    print("start text_train and dev_verdict")
    ...
    # text_train_verdict = open("text_train_verdict",'wb')
    # text_test_verdict = open("text_test_verdict",'wb')
    # labels_train_verdict = open("labels_train_verdict",'wb')
    # labels_test_verdict = open("labels_test_verdict",'wb')
    # pickle.dump(text_train,text_train_verdict)
    # pickle.dump(text_test,text_test_verdict)
    # pickle.dump(labels_train,labels_train_verdict)
    # pickle.dump(labels_test,labels_test_verdict)

    # print("loading pickles...")
    # text_train_verdict = open("text_train_verdict",'rb')
    # text_test_verdict = open("text_test_verdict",'rb')
    # labels_train_verdict = open("labels_train_verdict",'rb')
    # labels_test_verdict = open("labels_test_verdict",'rb')
    # text_train, labels_train = pickle.load(text_train_verdict), pickle.load(labels_train_verdict)
    # text_test, labels_test = pickle.load(text_test_verdict), pickle.load(labels_test_verdict)
    ...
    trainer, model = model_trainer(args.model_path,args.pretrained_path,args.num_label, train_dataset, test_dataset)
    trainer.train()
...
def main():
    ...
    parser.add_argument('--pretrained_path', type=str, help='/path/to/data')
    parser.add_argument('--wiki_path', type=str, help='/path/to/data')
    parser.add_argument('--num_label', type=int, help='num_label')
    ...
    # if not os.path.exists("annotations_dev"):
    print("start annotations_train and dev")
    ...
    if args.sample_nei:
        ...
        print("no sample NEI instances",flush=True)
    ...
    # annotations_train_file = open("annotations_train","wb")
    # annotations_dev_file = open("annotations_dev","wb")
    # pickle.dump(annotations_train,annotations_train_file)
    # pickle.dump(annotations_dev,annotations_dev_file)

    # annotations_train = open("annotations_train",'rb')
    # annotations_dev = open("annotations_dev",'rb')
    # annotations_train = pickle.load(annotations_train)
    # annotations_dev = pickle.load(annotations_dev)
