...
def claim_hypothesis_only_bias(annotations_train, annotations_dev, args):
    ...
    all_input_dev = []
    all_labels_dev = []
    if not os.path.exists("text_train.tmp"):
        print("start convert_evidence_into_tables")
        ...
        torch.save(text_train,"text_train.tmp")
        torch.save(labels_train,"labels_train.tmp")

        text_dev, labels_dev = tokenize_and_align_labels(text_dev, labels_dev, tokenizer)
        torch.save(text_dev,"text_dev.tmp")
        torch.save(labels_dev,"labels_dev.tmp")
    print("load text and labels...")
    text_train, labels_train = torch.load("text_train.tmp"),torch.load("labels_train.tmp")
    text_dev, labels_dev = torch.load("text_dev.tmp"),torch.load("labels_dev.tmp")
