...
def parse_args():
    ...
    if args.dataset_name is None and args.test_file is None :
        raise ValueError("Need either a task name or a test file.")
...
def main():
    ...
    for index in random.sample(range(len(test_dataset)), 2):
    ...
    samples_seen = 0
    out_f = open(os.path.join(args.output_dir,"res.jsonl"), "w")
    ...
    for step, batch in enumerate(tqdm(test_dataloader)):
        with torch.no_grad():
            ...
            for pred,label in zip(decoded_preds, decoded_labels):
                out_f.write(json.dumps({"pred":pred, "label":label})+"\n")
            # exit(0)
            if args.compute_metric:
