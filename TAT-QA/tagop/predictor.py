...
def main():
    mode = args.mode
    dev_itr = TaTQATestBatchGen(args, data_mode=mode, encoder=args.encoder)
    ...
    logger.info("Below are the result on "+mode+" set...")
    model.reset()
    model.avg_reset()
    pred_json = model.predict(dev_itr)
    json.dump(pred_json,open(os.path.join(args.save_dir, "pred_result_on_"+mode+".json"), 'w'))
    model.get_metrics()
